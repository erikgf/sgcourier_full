...
$(function(){
	//$(".select2").select2({language: "es", dropdownParent: $(".modal-content")});
	...
	for (var i = 0; i < data.length; i++){
		...
		html += `<tr>
					<th scope="row" class="text-center">
						<a title="VER" target="blank" href="../gestionar_ordenes/index.pronabec.admin.php?p_id=`+o.id+`" class="btn btn-xs btn-info"><i class="fa fa-eye"></i></a>	
...
var listar = function() {
	var postData = {
	  ...
	  $.post("../../controlador/pedidos.php?op=listar_pronabec", postData)
	      .done(fn)
	      .fail(fnFail);
};
...
var obtenerClientes = function() {
	  var fn = function(xhr){
	      ...
	      $("#txtclienteregistro").html(htmlSelect(datos));
	  };
	  ...
	  $.post("../../controlador/clientes.php?op=listar_select")
	      .done(fn)
	      .fail(fnFail);
};
