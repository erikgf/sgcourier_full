...
var guardarAsignaciones = function(){
	...
	//renderDataCantidades(DATA.cantidades);
	/*
	console.log({"id_colaborador" : idUsuarioAsignado, "ordenes" : arregloOrdenesAsignados});
	_alert($blkAlert, (parseInt(asignados) + parseInt(reemplazados))+ " asignaciones a "+nombrePersonal+" realizadas.", "success");
	$(".chkseleccion").prop("checked", false);
	*/
};
...
var asignar = function(){
	...
	for (var i = $trs.length - 1; i >= 0; i--) {
		...
		if (($tr_dataset.estadoactual == "G" || $tr_dataset.estadoactual == "N") && $tr_dataset.codigoremito == codigoRemitoBuscar.trim()){
			var $inputCheck = $tr.children[0].children[0];
			$inputCheck.checked = !$inputCheck.checked;
			cambiarTr($tr, $tr_dataset.estadoactual, $inputCheck, false);
			notfound = false;
			break;
		}
	};
